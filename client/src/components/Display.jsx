...
export default function Display({ contract, account }) {
  ...
  const getFiles = useCallback(async () => {
    if (!contract || !account) {
      setMessage({ type: 'error', text: 'Contract not initialized or account not connected' });
      return;
    }
    ...
    try {
      ...
      if (otherAddress && otherAddress !== account) {
        try {
          ...
          if (!hasAccess) {
            setMessage({ type: 'error', text: "You don't have access to view these files" });
            setLoading(false);
            return;
          }
        } catch (error) {
      ...
      setMessage({ 
        type: 'error', 
        text: error.message || "Failed to load files. Check console for details." 
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="files-grid">
          {files.map((file, index) => (
            <div key={index} className="file-card">
              <div 
                className={`file-preview ${!isImage(file.fileType) ? 'file-icon' : ''}`}
                ...
                {isImage(file.fileType) ? (
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentNode.innerHTML = '<i class="fas fa-image"></i>';
                    }}
                  />
                ) : isVideo(file.fileType) ? (
                  <video src={file.url} />
                ) : (
                  <i className={`fas ${getFileIcon(file.fileType)}`} style={{ fontSize: '3rem' }}></i>
                )}
              </div>
              
              <div className="file-info">
                <h4 title={file.name || 'Unnamed file'}>
                  {file.name ? (file.name.length > 20 ? file.name.substring(0, 20) + '...' : file.name) : 'Unnamed file'}
                </h4>
          ...
          <div className="preview-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="preview-body">
              {isImage(previewFile.fileType) ? (
                ...
                <video controls src={previewFile.url} style={{ maxWidth: '100%', maxHeight: '60vh' }} />
              ) : isAudio(previewFile.fileType) ? (
                <audio controls src={previewFile.url} />
              ) : (
                <div className="file-icon-large">
                  <i className={`fas ${getFileIcon(previewFile.fileType)}`} style={{ fontSize: '5rem' }}></i>
            ...
            <div className="preview-footer">
              <div className="file-details">
                <span><strong>Type:</strong> {previewFile.fileType || 'Unknown'}</span>
                <span><strong>Size:</strong> {formatFileSize(Number(previewFile.size))}</span>
                <span><strong>Uploaded:</strong> {previewFile.timestamp ? 
                  new Date(Number(previewFile.timestamp) * 1000).toLocaleString() : 
                  'Unknown'
                }</span>
              </div>
            </div>
